...
def send_email(request):
       ...
       frommail = request.POST.get('from',"")
       if subject and  message and frommail:
              try:
                     send_mail(subject,message,frommail,['admin@gmail.com'])
                     return HttpResponse('everything is undercontrol')
              except BadHeaderError:
                     return HttpResponse('note vilad email operation')
                     
       else:
              return HttpResponse('make sure all field is entered ')
...
class CheckoutView(View):
       ...
       def post(self , *args,**kwargs):
           ...
           try:
              oreder=Order.objects.get(user = self.request.user,ordered=False)
              if form.is_valid():
